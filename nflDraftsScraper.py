...
def name(player, row):
    ...
    nameArr = row.contents[1].get('href').split('/')
    player['forChecking'] = nameArr[2]
    names = row.contents[1].text.split()
    # global globalName;
    # globalName = names[0].lower() + names[1].lower();
    ...
    for tr in table.findAll('tr',{'class':evenOdd}):
        for row in tr.findAll("td"):
            ...
            if (i%13 == 0):
                ...
                # playerDraft['optionID'] = counter
                if (playerDraft):
                    if (playerDraft['team'].replace(" ", "").lower() == player['team'].lower()):
                        playerDraft['status'] = player['status']
                    else:
                        if (playerDraft['status']=='N/A'):
                            playerDraft['status'] = 'OTHER_TEAM'

def rosterJsonAddAll(soupR, rosterJson, urlR, drafts):
    counter = 0;
    for table in soupR.findAll("table"):
        # headerR = {}
        # rosterJson.append(headerR)
        i = 0
        scrapContents(rosterJson, drafts, 'odd', table);
        # for trOdd in table.findAll('tr',{'class':'odd'}):
        #     for row in trOdd.findAll("td"):
        #         i+=1
        #         if (i%13==1):
        #             count = 0
        #             player = {}
        #             rosterJson.append(player)
        #         count += 1
        #         scrapContents(rosterJson, drafts, i, player, count,);

        scrapContents(rosterJson, drafts, 'even', table);
        counter+=1;
        # if (i != 0):
        #     headerR['numPlayers'] = i/13
        # else:
        #     rosterJson.remove(headerR)
            # if (soup.findAll('select', {'name':'filter'})['value']!=None): #&&
            #     #soup.findAll('select', {'name':'filter'})['value'] == urlR[-4:])
            #     headerR['teamName'] = soup.findAll('select', {'name':'filter'})[urlR[-4:]]
...
urlListRoster = []
#ComboBox!
# selectBar = soup.findAll('select', {'name':'teamId'})  JUST FOR FUTURE REFERENCE
for option in soup.findAll('option'):
    urlListDrafts.append(urlBaseDrafts+option['value']+"&type=team")
    urlListRoster.append(urlBaseRoster+option['value'])


#this is for 2015 drafts (in different site)
url2015_init = "http://www.nfl.com/draft/2015/tracker#dt-tabs:dt-by-team/dt-by-name-input:j/dt-by-round-input:4/dt-by-team-input:ari";
page2015_init = urllib2.urlopen(url2015_init).read();
soup2015 = BeautifulSoup(page2015_init, "html.parser");

url2015_base = "http://www.nfl.com/widget/draft/2015/tracker/teams/arizonacardinals/profile?year=2015&team="
url2015_list = []

#TO DO: 2015 SCRAPING
# for option in soup2015.findAll('option', {'class': 'filter-option'}):
#     url2015_list.append(url2015_base+option['value']+"&random=1458238920000");
...
# with open('testingR.json', 'w') as outfile:
#     json.dump(rosterJson, outfile, ensure_ascii=False, indent=4)
with open('combinedRosterDraft.json', 'w') as outfile:
    json.dump(draftsJson, outfile, ensure_ascii=False, indent=4)

#how should I use the data gathered from this program??
